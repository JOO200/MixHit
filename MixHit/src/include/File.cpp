 ...
 */
AdvFile::AdvFile(std::string path, uint8_t type) {
	m_path = path;
	m_type = type;
} // File
 ...
 */
std::string AdvFile::getContent(bool base64Encode) {
	uint32_t size = length();
	ESP_LOGD(LOG_TAG, "AdvFile:: getContent(), path=%s, length=%d", m_path.c_str(), size);
	if (size == 0) {
		return "";
	}
 ...
 * @return The content of the file.
 */
std::string AdvFile::getContent(uint32_t offset, uint32_t readSize) {
	uint32_t fileSize = length();
	ESP_LOGD(LOG_TAG, "AdvFile:: getContent(), name=%s, fileSize=%d, offset=%d, readSize=%d",
