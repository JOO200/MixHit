 ...
 * @return N/A.
 */
void AdvSPI::init(int mosiPin, int misoPin, int clkPin, int csPin) {
 ...
 */
void AdvSPI::setHost(spi_host_device_t host) {
	m_host = host;
} // setHost
 ...
 */
void AdvSPI::transfer(uint8_t* data, size_t dataLen) {
	assert(data != nullptr);
	assert(dataLen > 0);
#ifdef DEBUG
 ...
 */
uint8_t AdvSPI::transferByte(uint8_t value) {
	transfer(&value, 1);
	return value;
} // transferByte
