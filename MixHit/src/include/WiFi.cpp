 ...
 * @return N/A.
 */
void AdvWiFi::addDNSServer(const std::string& ip) {
	addDNSServer(ip.c_str());
} // addDNSServer


void AdvWiFi::addDNSServer(const char* ip) {
...
void AdvWiFi::addDNSServer(ip_addr_t ip) {
 ...
 * @return N/A.
 */
void AdvWiFi::setDNSServer(int numdns, const std::string& ip) {
	setDNSServer(numdns, ip.c_str());
} // setDNSServer


void AdvWiFi::setDNSServer(int numdns, const char* ip) {
	ip_addr_t dns_server;
	if(inet_pton(AF_INET, ip, &dns_server)) {
...
void AdvWiFi::setDNSServer(int numdns, ip_addr_t ip) {
	ESP_LOGD(LOG_TAG, "Setting DNS[%d] to %d.%d.%d.%d", m_dnsCount, ((uint8_t*)(&ip))[0], ((uint8_t*)(&ip))[1], ((uint8_t*)(&ip))[2], ((uint8_t*)(&ip))[3]);
	init();
 ...
 */
uint8_t AdvWiFi::connectAP(const std::string& ssid, const std::string& password, bool waitForConnection, wifi_mode_t mode){
 ...
 */
/* STATIC */ esp_err_t AdvWiFi::eventHandler(void* ctx, system_event_t* event) {
	...
	AdvWiFi *pWiFi = (AdvWiFi *)ctx;   // retrieve the WiFi object from the passed in context.
 ...
 */
tcpip_adapter_ip_info_t AdvWiFi::getApIpInfo() {
	//init();
	tcpip_adapter_ip_info_t ipInfo;
 ...
 */
std::string AdvWiFi::getApNetmask(){
	tcpip_adapter_ip_info_t ipInfo = getApIpInfo();
	char ipAddrStr[30];
 ...
 */
std::string AdvWiFi::getApGateway(){
	tcpip_adapter_ip_info_t ipInfo = getApIpInfo();
	char ipAddrStr[30];
 ...
 */
struct in_addr AdvWiFi::getHostByName(const std::string& hostName) {
	return getHostByName(hostName.c_str());
} // getHostByName


struct in_addr AdvWiFi::getHostByName(const char* hostName) {
 ...
 */
tcpip_adapter_ip_info_t AdvWiFi::getStaIpInfo() {
	tcpip_adapter_ip_info_t ipInfo;
 ...
 * @return The Netmask IP.
 */
std::string AdvWiFi::getStaNetmask(){
	tcpip_adapter_ip_info_t ipInfo = getStaIpInfo();
	char ipAddrStr[30];
	inet_ntop(AF_INET, &ipInfo.netmask.addr, ipAddrStr, sizeof(ipAddrStr));
	return std::string(ipAddrStr);
 ...
 * @return The Gateway IP.
 */
std::string AdvWiFi::getStaGateway(){
	tcpip_adapter_ip_info_t ipInfo = getStaIpInfo();
	char ipAddrStr[30];
	inet_ntop(AF_INET, &ipInfo.gw.addr, ipAddrStr, sizeof(ipAddrStr));
	return std::string(ipAddrStr);
 ...
 */
/* PRIVATE */ void AdvWiFi::init() {

	// If we have already started the event loop, then change the handler otherwise
	// start the event loop.
	if (m_eventLoopStarted) {
		esp_event_loop_set_cb(AdvWiFi::eventHandler, this);   // Returns the old handler.
	} else {
		esp_err_t errRc = ::esp_event_loop_init(AdvWiFi::eventHandler, this);  // Initialze the event handler.
		if (errRc != ESP_OK) {
 ...
 */
std::vector<WiFiAPRecord> AdvWiFi::scan() {
 ...
 * @return N/A.
 */
void AdvWiFi::startAP(const std::string& ssid, const std::string& password, wifi_auth_mode_t auth) {
 ...
 * @return N/A.
 */
void AdvWiFi::startAP(const std::string& ssid, const std::string& password, wifi_auth_mode_t auth, uint8_t channel, bool ssid_hidden, uint8_t max_connection) {
 ...
 */
void AdvWiFi::setWifiEventHandler(WiFiEventHandler* wifiEventHandler) {
 ...
 * @return N/A.
 */
void AdvWiFi::setIPInfo(const std::string& ip, const std::string& gw, const std::string& netmask) {
	setIPInfo(ip.c_str(), gw.c_str(), netmask.c_str());
} // setIPInfo



void AdvWiFi::setIPInfo(const char* ip, const char* gw, const char* netmask) {
	uint32_t new_ip;
	uint32_t new_gw;
 ...
 */
void AdvWiFi::setIPInfo(uint32_t ip, uint32_t gw, uint32_t netmask) {
	init();
